...
LUA_API void lua_setstring(lua_State *L, void* str)
{
  GCstr *s = ( GCstr* ) str;
  lj_gc_check(L);
  setstrV(L, L->top, s);
...
LUA_API void* lua_getstring(lua_State *L, size_t length, char** string)
{
    GCstr* data = lj_str_new(L, "", length);
    *string = ( char* ) data + sizeof( GCstr );
    return data;
}
