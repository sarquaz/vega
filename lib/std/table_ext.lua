...
function hash(t)
    ...
    for key, value in pairs(t) do
        if type(value) == 'table' then value = table.hash(value) end

        table.insert(array, key .. tostring(value))

    end
    
    for _, value in ipairs(table.sort(array)) do
        hash = hash .. value
    end
    
    return md5.hexsum(hash)
end
