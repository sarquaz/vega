...
function elems (l)
  assert(l, "nil passed instead of list")
  local n = 0
  return function (l)
           n = n + 1
           if n <= #l then
...
--- Return a string comprised as a concatenation of list elements
-- @param l list
-- @param s separator 
...
function join(l, s)
    ...
    for _, v in ipairs(l) do 
        assert(type(v) ~= 'table', 'can only operate on value types')
        r = r .. v .. s
    end
    
    return r:sub(0, r:len() - s:len())
end
